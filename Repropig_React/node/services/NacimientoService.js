...
class NacimientoService {
    ...
    async getById(id) {
        const player = await NacimientoModel.findByPk(id) 
    ...
    async update(id,data) {
        const result = await NacimientoModel.update(data,{where: {id}})
    ...
    async delete(id) {
        const deleted = await NacimientoModel.destroy({ where: {id}})
